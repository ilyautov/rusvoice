...
Русская озвучка для клонированного голоса: **слой правки текста и способ увидеть его работу
до синтеза.**
...
```bash
pip install .                    # из корня репозитория
pip install ".[accent,ui]"       # + ударения (RUAccent) и экран
pip install ".[voice,accent]"    # + собственно синтез (F5)
rusvoice --help
```
...
**Движок [Pygmalion](../README.md) пакету не нужен ни для чего, включая синтез.** Рецепт
клона живёт здесь же — [`clone.py`](clone.py). Так было не всегда: он лежал в
